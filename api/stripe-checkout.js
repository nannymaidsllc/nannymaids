// NannyMaids — Stripe Checkout Session Creator (Vercel)
...
module.exports = async (req, res) => {
  ...
  if (!STRIPE_SECRET) {
    ...
    return res.status(500).json({ error: 'Payment system not configured. Please contact us at (813) 400-3679.' });
  }
  ...
  if (!amountCents || amountCents < 50) {
    return res.status(400).json({ error: 'Could not calculate a valid price. Please go back and check your selections.' });
  }
  ...
  try {
    ...
    if (!stripeRes.ok) {
      ...
      return res.status(500).json({ error: session.error?.message || 'Stripe returned an error. Please try again.' });
    }
