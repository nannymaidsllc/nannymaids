...
module.exports = async (req, res) => {
  ...
  if (CUSTOM_SERVICES.includes(service)) return res.status(400).json({ error: 'custom', message: 'This service requires a custom quote.' });

  const amountCents = calculateAmount(service, options);
  if (!amountCents || amountCents < 50) return res.status(400).json({ error: 'Could not calculate price. Please go back and check your selections.' });

  const description = [service, options.beds ? `${options.beds} bed` : null, options.baths ? `${options.baths} bath` : null].filter(Boolean).join(' | ');
  ...
  try {
    ...
    return res.status(502).json({ error: 'Could not reach Stripe. Call (813) 400-3679.' });
  }
};
