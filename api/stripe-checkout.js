...
function calculateAmount(service, options) {
  ...
  if (options.addons && Array.isArray(options.addons)) {
    for (const addon of options.addons) { if (ADDON_PRICES[addon]) total += ADDON_PRICES[addon]; }
  }
  return total;
}
module.exports = async (req, res) => {
  ...
  try {
    const stripeRes = await fetch('https://api.stripe.com/v1/checkout/sessions', {
      method: 'POST',
      headers: { 'Authorization': `Basic ${Buffer.from(STRIPE_SECRET + ':').toString('base64')}`, 'Content-Type': 'application/x-www-form-urlencoded' },
